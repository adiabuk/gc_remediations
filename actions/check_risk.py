...
class CheckRisk(Action):
    def run(self, down_device, environment):
        print(f"Checking to see if risk is too high in {environment}")
        command = f'/opt/nagios_checks/check_nrpe -2 -t10 -H {down_device} -c get_risk_{environment}'
        ...
        print(f"Risk is {risk}, result={result} for {environment}")
